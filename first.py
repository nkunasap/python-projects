...
if sys._version_info <= 2:import http.client as httplib # type: ignore
...
class FaceBoom(object):
    ...
    def __init__(self):
        self.useProxy = None
        self.br = mechanicalsoup.browser() # type: ignore
    ...
    def check_proxy(proxy):
          ...
          try:
            r = requests.get('https://www.wikipedia.org',proxies=proxies, timeout=5) # type: ignore
    ...
    def get_profile_id(self, target_profile):
        try:
            ...
            idis = idre.search(con).group() # type: ignore
            print(wi+"\n["+gr+"+"+wi+"]"+gr+" Target Profile"+wi+" ID: "+yl+idis+wi) # type: ignore
        except Exception:
...
==================================\n"""+wi)
    ...
    @staticmethod
    def updateFaceBoom():
        if not os.path.isfile(versionPath): # type: ignore
             errMsg("Unable to check for updates: please re-clone the script to fix this problem")
             sys.exit(1)
        write("[~] Checking for updates...\n")
        conn = httplib.HTTPSConnection("raw.githubusercontent.com")
        conn.request("GET", "C:/Users/MICHEAL/Desktop/asssign/FaceBoom-master/FaceBoom-master/core/version.txt")
        repoVersion = conn.getresponse().read().strip().decode()
        with open(versionPath) as vf: # type: ignore
        ...
        if repoVersion == currentVersion:write("  [*] The script is up to date!\n")
                ...
                with open("faceboom.py", "w") as  faceBoomScript: # type: ignore
                   faceBoomScript.write(newCode)
                with open(versionPath, "w") as ver: # type: ignore
...
def Main():
   ...
   if update:
    ...
    sys.exit(1)
        ...
        if wordlist:
            if not os.path.isfile(wordlist): # type: ignore
        ...
        for passwd in passwords:
                ...
                if retCode:
                    ...
                    loop = loop + 1 if not single_passwd else "~" # type: ignore
        else:
                if single_passwd:
                    print(yl+"\n["+rd+"!"+yl+"] Sorry: "+wi+"The Password[ "+yl+passwd+wi+" ] Is Not Correct"+rd+":("+yl+"!"+wi)# type: ignore
